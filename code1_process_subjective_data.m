...
path_subjective_data = '..\database\subjective_data';
...
for cnt = 1:length(file_lists)
    if (length(file_lists(cnt).name) >3) && (strcmp(file_lists(cnt).name(end-3:end), '.csv') == 1)
        ...
        scores_original1(:,subject_num) = temp_data(:,2);
        scores_original2(:,subject_num) = temp_data(:,3);
    end
end
...
% Distribution of subjective quality scores ({\alpha} = 0.25)
index1 = 0;
index2 = 0;
for i = 1:450
    if (alpha(i,1)==0.25)
...
xlabel('scores');
ylabel('Image Number');
title('Distribution of subjective quality scores ({\alpha} = 0.25)');

% Distribution of subjective quality scores ({\alpha} = 0.5)
index1 = 0;
index2 = 0;
for i = 1:450
    if (alpha(i,1)==0.5)
...
xlabel('scores');
ylabel('Image Number');
title('Distribution of subjective quality scores ({\alpha} = 0.5)');


% Distribution of subjective quality scores ({\alpha} = 0.75)
index1 = 0;
index2 = 0;
for i = 1:450
    if (alpha(i,1)==0.75)
...
DistortionIndex3 = [DistortionType3 DistortionType3 DistortionType3 DistortionType3 DistortionType3 DistortionType3 DistortionType3 DistortionType3 DistortionType3 DistortionType3 DistortionType3 DistortionType3 DistortionType3 DistortionType3 DistortionType3];
numberTotal = zeros(1,10);
scoresAfterRemoveOutliers = SCORE.*acceptIndex.*(DistortionIndex3==1);
for i = 1:subject_num
    for j = 1:10
        numberTotal(j) = numberTotal(j)+length(find(scoresAfterRemoveOutliers(:,i)==j));
    end
...
xlabel('scores');
ylabel('Image Number');
title('Distribution of subjective quality scores ({\alpha} = 0.75)');
...
hole on 
h4 = histogram(MOSz(find(DistortionType3==1)))
...
hold on
plot(x,y,'LineWidth',1.5)
legend('Entire database','{\alpha} = 0.25','{\alpha} = 0.5','{\alpha} = 0.5','Gaussian fitting ({\sigma}=21.07)','Gaussian fitting ({\sigma}=16.36)','Gaussian fitting ({\sigma}=13.24)')
grid on