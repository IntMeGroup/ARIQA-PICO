...
net = lpips.LPIPS(net=opt.net, lpips=False).cuda()
...
with torch.no_grad():
    for i, (img_dis, img_ref, mos) in enumerate(test_loader):
        ...
        img_dis.to(dev)
        img_ref.to(dev)
        loss,res = net.forward(img_dis, img_ref, retPerLayer=True)
        ...
        if i == 0:
            test_mos_predict_res = [[] for i in range(len(res))]

        for k in range(len(res)):
            test_mos_predict_res[k].append(np.reshape(mos_predict_res[k].cpu().numpy(),(1)))
...
i=0
for r in test_mos_predict_res:
    iqa = IQAPerformance(test_mos_origin,r)
    i=i+1
    print(i)
    print(iqa.compute())
