...
if __name__ == '__main__':
    ...
    mat_file_name = '../../database/CFIQAMOS/MOS.mat'
    # load IQA mos from .mat file
    ...
    for i in range(450):
        count1 = i+1
        img_name_current = f'{count1}_{count2}_{count3}-{count4}.png'  #The list of image file names
        disImgName.append('/img_Mixed/'+img_name_current)         #Mixed
        refImgName.append('/img_AR/reference/'+img_name_current)  #AR
        mos.append(CFIQA_mos_value[i][0])
        refImgName2.append('/img_BG/captured/'+img_name_current)  #BG
        mos2.append(CFIQA_mos_value[i+450][0])
        count3 += 1
        if(count3 > 15):
            count3 = 1
            count4 += 1
