...
def spatial_sal(in_tens, sal, keepdim=True):
    sal = nn.functional.interpolate(sal.cuda(), size=(in_tens.shape[2], in_tens.shape[3]), mode='bicubic', align_corners=False)
...
class LPIPS(nn.Module):
    def __init__(self, pretrained=True, net='alex', version='0.1', lpips=True, spatial=False, 
        ...
        if(lpips):
            ...
            if(pretrained):
                ...
                self.load_state_dict(torch.load(model_path, map_location='cuda'), strict=False)          
    ...
    def forward(self, in0, in1, sal=None, retPerLayer=False, normalize=False):
        ...
        outs0, outs1 = self.net.forward(in0_input.cuda()), self.net.forward(in1_input.cuda())
        feats0, feats1, diffs = {}, {}, {}
        ...
        if(self.edge):
            outs0_e, outs1_e = self.net_e.forward(in0_input.cuda()), self.net_e.forward(in1_input.cuda())
...
class ScalingLayer(nn.Module):
    ...
    def forward(self, inp):
        return (inp.cuda() - self.shift.cuda()) / self.scale.cuda()
...
class BCERankingLoss(nn.Module):
    ...
    def forward(self, d0, d1, judge):
        ...
        return self.loss(self.logit, per.cuda())
...
class BCEScoringLoss(nn.Module):
    ...
    def forward(self, d, y):
        ...
        # self.logit = d
        return self.loss(self.logit, y.cuda())
